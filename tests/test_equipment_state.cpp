...
int main() {
    ...
    assert(nearlyEqual(computeEquipBurden(session, content), 0.0f));
    ...
    assert(nearlyEqual(view.burden, 2.0f));
    ...
    assert(view.twoHanded);
    assert(nearlyEqual(view.burden, 6.5f));
    ...
    assert(view.rightHandWeaponId.empty());
    assert(view.leftHandWeaponId.empty());
    assert(nearlyEqual(view.burden, 0.0f));
