...
InventoryCaptureState& ensureCaptureState(GameRegistry& registry) {
    auto& ctx = registry.ctx();
    if (!ctx.contains<InventoryCaptureState>()) {
        ctx.emplace<InventoryCaptureState>();
    }
    return ctx.get<InventoryCaptureState>();
}
...
void initializeRuntimeInventory(GameRegistry& registry) {
    ...
    (void)ensureCaptureState(registry);
}
...
void updateRuntimeInventory(GameRegistry& registry,
    ...
    auto& captureState = ensureCaptureState(registry);
...
void resetRuntimeInventory(GameRegistry& registry) {
    ensureMenuState(registry) = InventoryMenuState{};
    ensureCaptureState(registry) = InventoryCaptureState{};
}
...
void clearRuntimeInventory(GameRegistry& registry) {
    ...
    if (ctx.contains<InventoryCaptureState>()) {
        ctx.erase<InventoryCaptureState>();
    }
}
